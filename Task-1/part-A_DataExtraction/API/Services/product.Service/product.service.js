...
const _providerChecker = (shipmentMethodDescription) =>
  shipmentMethodDescription === "موجود در انبار دیجی‌کالا";
...
async function productExtractor(data) {
  ...
  const _productAdder = async () => {
    await products.forEach(async (item) => {
      let _product = await Product.findOne({ DK_ID: item.id }).catch((err) =>
        console.log(err)
      );
